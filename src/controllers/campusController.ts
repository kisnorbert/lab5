...
export default class CampusController {
  ...
  public async editCampus(req: Request, res: Response, next: NextFunction) {
    try {
      const campusId = req.params.id;
      const updatedData = req.body;
  
      const campusDTO: ICampusDTO = {
        id: campusId,
        ...updatedData
      };
  
      const result = await this.campusServiceInstance.editCampus(req.body as ICampusDTO) as Result<ICampusDTO>;;
